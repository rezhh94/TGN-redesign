...
- **I: Token-konsolidering — display-tittel, mono-tracking, fin-radius (2026-07-07):** samlet spredte typo-verdier til tre nye tokens, verifisert live på hele forsiden. Full audit + ferskt lamalama-ekstrakt i `forside-inkonsistens-rapport.md` (rota).
  - **`--display-track: 0.01em`** — ÉN kilde for alle kondenserte TGS-caps-titler (8 stk: approach, what-build tittel + pilar, work-proof, wp-case, process tittel + heading, contact ×2). Var før spredt 0 / 0.01 / −0.012em, og contact-vekt 800. Nå: **display-tittel = `var(--font-display)` + `font-weight: 700` + `letter-spacing: var(--display-track)`** overalt — aldri en løs vekt/tracking. Hero-wordmarken (bokstavstablet TIGON/STUDIO, 900) er egen sak og bruker den ikke.
  - **`--mono-track: 0.08em`** — ÉN kilde for alle uppercase Caleb-Mono-labels (eyebrows/tags/meta, 19 stk). Var før ≥7 verdier (0.04–0.14em). Regel: **mono-label = `var(--font-mono)` + `text-transform: uppercase` + `letter-spacing: var(--mono-track)`**. CTA-label bruker bevisst `--cta-tracking` (−0.02em, lamalama-kalibrert), ikke denne. Tracking forblir positiv (bevisst avvik fra lamalamas −0.02em — for stramt på Caleb/versaler).
  - **`--radius-fine: 2px`** — bittesmå elementer (hamburger-strek, svart/hvit tag-brikker 5×9px) der 4px leser for rundt. **Radius-skalaen er nå tre trinn:** `--radius-fine 2` (fin) / `--radius 4` (knapp/CTA/kort/ramme) / `--radius-chrome 10` (flytende chrome). Piller 999 kun for rene runde toggles/prikker. Velg ett trinn, aldri en px imellom.
  - **`--color-signal` (#c44414, bannlyst oransje) SLETTET** fra `tokens.css` — var ubrukt. Trengs signal-farge senere: bruk `--pine`, eller re-introduser eksplisitt etter godkjenning.
  - **Farge bekreftet stram:** presis skann viste ingen levende hardkodede token-dubletter (de tilsynelatende near-dupes lå i kommentarer / gradient-stopp / selve token-defs). Rene svart/hvit tag-brikker (`#000`/`#fff`) beholdt bevisst utenfor cream/blekk-paletten for pop.
