...
- `Effekt som kan måles.` and its supporting sentence are the opening phase.
  ...
  reading and blur out before the central Tigon-owned image appears; there is
  no vertical title translation, mask or scale animation. Only opacity and
  blur animate on the text, and the same copy is not repeated on the image.
...
- The wide scene ends at `4.6 * innerHeight`, or `3.9 * innerHeight` for touch,
  ...
  `0`. It then
  completes the title-to-image handoff before mapping the
...
- Below 768px, one pinned phone scene keeps the central image fixed while all
  ...
  with no overlap and reserves `0–.66` for the fixed blur-to-image handoff; phone
